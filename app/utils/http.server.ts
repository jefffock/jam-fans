...
export function getRequiredParam(params: Record<string, string | undefined>, key: string) {
...
export function assertIsDelete(request: Request, message = 'Method not allowed') {
	if (!isDelete(request)) {
		throw notAllowedMethod(message)
	}
}
