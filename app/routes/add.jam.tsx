...
export const loader = async ({ request }: LoaderFunctionArgs) => {
	...
	if (queryParams?.jamid) {
		const { data } = await supabase.from('versions').select('*').eq('id', JSON.parse(queryParams.jamid)).single()
		jam = data
	}
	if (queryParams?.song) {
		initialSong = queryParams.song
		const { data: songObj } = await supabase.from('songs').select('*').eq('song', queryParams.song).single()
		initialSongObj = songObj
	}
	if (queryParams?.artist) {
		const { data: artistObj } = await supabase.from('artists').select('*').eq('artist', queryParams.artist).single()
		initialArtist = artistObj
	}
	if (queryParams?.artistUrl) {
		const { data: artistObj } = await supabase.from('artists').select('*').eq('url', queryParams.artistUrl).single()
		initialArtist = artistObj
	}
	if (queryParams?.date) {
	...
	//get songs
	const { data: songs } = await supabase.from('songs').select('song, artist').order('song', { ascending: true })
	const { data: sounds } = await supabase.from('sounds').select('label, text').order('label', { ascending: true })
	//get artists
	let { data: artists } = await supabase.from('artists').select('*').order('name_for_order', { ascending: true })
...
export async function action({ request, params }: ActionFunctionArgs) {
	...
	async function addOnePoint(profileName: string) {
		const { error } = await supabase.rpc('add_one_point', {
	...
	async function addTenPoints(profileName: string) {
		const { error } = await supabase.rpc('add_ten_points', {
			username: profileName,
		})
		if (error) {
	...
	async function addRatingCountToArtist(artistId: string) {
		const { error } = await supabase.rpc('add_rating_count_artist', {
			artistid: artistId,
		})
		if (error) {
	...
	async function addRatingCountToSong(songId: string) {
		...
		const { error } = await supabase.rpc('increment_song_rating_count', {
			songid: song_id,
		})
		if (error) {
	...
	async function addRatingCountToVersion(versionId: string) {
		...
		const { error } = await supabase.rpc('increment_version_rating_count', {
	...
	async function calcAverageForVersion(versionId: string) {
		...
		const { error } = await supabase.rpc('calc_average', {
			versionid: version,
		})
		if (error) {
	...
	let sounds: string[] = []
	if (values.sounds) {
		String(values.sounds)
			.split(',')
			.forEach((sound: string) => {
				sounds.push(sound)
			})
	}
	let jam
	...
	if (values.songObj) {
		...
		const { data, error } = await supabase.from('songs').select('*').eq('song', values['new-song']).single()
		songObj = data
	}
	if (_action === 'clear') {
		return {
	...
	if (_action === 'new-song') {
		...
		const { data, error } = await supabase.from('songs').insert({ song: values['new-song'], artist: values.artist })
		if (profile?.name) {
	...
	if (_action === 'add-and-rating') {
		...
		if (data && data?.length > 0) {
			...
			const { data: dataFromAddRating, error } = await supabase
				.from('ratings')
				.insert({
					user_id: profile?.id,
					version_id: jamId,
...
export default function AddJam() {
	...
	const [songs, setSongs] = useState<any[] | null>(initialSongs ?? null)
	...
	const sortedSongs = useMemo(() => {
		return artist
			? songs?.sort((a: SongInSonglist, b: SongInSonglist) => {
	...
	function handleArtistChange(artist: Artist) {
		...
		if (artist && year && !date && artist.artist !== 'Squeaky Feet' && artist.artist !== 'Houseplant') {
			//fetch shows
	...
	useEffect(() => {
		...
		if (!actionData?.body?.includes('action complete') && artist) {
			...
			if (
				...
				setShows([])
				let urlToFetch = '/getShows?artist=' + artist.artist + '&song=' + songSelected
				fetcher.load(urlToFetch)
	...
	function handleShowChange(show: Show) {
		if (show) {
			...
			if (useApis && artist && artist.artist !== 'Squeaky Feet' && artist.artist !== 'Houseplant') {
				let urlToFetch = '/getSetlist?artist=' + artist.artist + '&date=' + show.showdate
				fetcher.load(urlToFetch)
			}
			setShow(show)
	...
	useEffect(() => {
		...
		if (
			data?.jam &&
			data.jam !== 'not on jf' &&
			(!jam || typeof jam === 'string' || data.jam.id !== jam.id) &&
			songSelected === data.jam.song_name
		) {
			setJam(data.jam)
		}
	...
	function clearSong() {
		...
		if (!setlist) {
			...
			if (show) {
				...
				urlToFetch = '/getSetlist?artist=' + artist?.artist + '&date=' + show?.showdate
			} else if (date) {
	...
	function handleSoundsChange(e: React.ChangeEvent<HTMLInputElement>) {
		...
		if (soundsSelected?.includes(e.target.value)) {
			let newSoundsSelected = soundsSelected.filter((sound) => sound !== e.target.value)
	...
	return (
		<Form method="post" className="h-screen pb-40">
			<div className="flex flex-col space-y-4 p-4 pb-40 max-w-xl mx-auto h-full">
				<p className="text-center mt-4 text-xl">Add {profile ? 'and/or Rate ' : ''}a Jam</p>
				...
				<input type="hidden" name="song" value={songSelected ?? undefined} />
				<input type="hidden" name="date" value={date ?? undefined} />
				<input type="hidden" name="location" value={location ?? undefined} />
				<input type="hidden" name="jam" value={JSON.stringify(jam)} />
				<input type="hidden" name="sounds" value={soundsSelected ?? undefined} />
				<input type="hidden" name="rating" value={rating ?? undefined} />
				<input type="hidden" name="songObj" value={JSON.stringify(songObj)} />
				...
				{!artist && (
					<div className="max-h-40 max-w-sm">
						<Listbox value={artist} onChange={handleArtistChange}>
							{({ open }) => (
								<>
				...
				{artist && (
					<div className="flex justify-between text-sm">
						...
						<button type="button" onClick={clearArtist}>
							Change Artist
						</button>
					</div>
				)}
				...
				{useApis &&
					...
					[
						...
						<div className="max-h-40">
							<Listbox value={show} onChange={handleShowChange}>
								{({ open }) => (
									<>
										<Listbox.Label className="block text-md font-medium text-gray-700">
											Shows with a
				...
				{showAddSong && (
					<div>
						<InfoAlert
							title={`No songs containing "${query && query !== '' ? query : songSelected}" found`}
							description={`Click the button below to add "${
								query && query !== '' ? query : songSelected
							}" to Jam Fans`}
						/>
						<label htmlFor="new-song" className="block text-md font-medium text-gray-700">
							New song to add
						</label>
						<div className="mt-1">
							<input
						...
						<p className="mt-2 text-sm text-gray-500" id="new-song-description">
				...
				{useApis && artist && !date && !songSelected && (
					<div className="max-h-40 max-w-xs">
						<Listbox value={year} onChange={handleYearChange}>
							{({ open }) => (
								<>
									<Listbox.Label className="block text-md font-medium text-gray-700">
										Year
									...
									<div className="relative mt-1">
										<Listbox.Button className="relative w-full cursor-default rounded-md border border-gray-300 bg-white py-2 pl-3 pr-10 text-left shadow-sm focus:border-cyan-500 focus:outline-none focus:ring-1 focus:ring-cyan-500 sm:text-sm h-10">
											<span className="block truncate">{year || 'Choose year'}</span>
											<span className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2">
												<ChevronUpDownIcon
													className="h-8 w-8 text-gray-400"
													aria-hidden="true"
				...
				{useApis &&
					...
					year &&
					!songSelected &&
					(!fetcher || (fetcher && fetcher.state && fetcher.state !== 'loading')) && (
						<div className="max-h-40">
							<Listbox value={show} onChange={handleShowChange}>
								{({ open }) => (
									<>
				...
				{useApis && setlist && date && !songSelected && (
					<div className="max-h-40">
						<Listbox value={songSelected || ''} onChange={setSongSelected}>
							{({ open }) => (
								<>
									<Listbox.Label className="block text-md font-medium text-gray-700">
									...
									<div className="relative mt-1">
										<Listbox.Button className="relative w-full cursor-default rounded-md border border-gray-300 bg-white py-2 pl-3 pr-10 text-left shadow-sm focus:border-cyan-500 focus:outline-none focus:ring-1 focus:ring-cyan-500 sm:text-sm h-10">
											<span className="block truncate">{songSelected || 'Choose song'}</span>
											<span className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2">
												<ChevronUpDownIcon
													className="h-8 w-8 text-gray-400"
													aria-hidden="true"
				...
				{!useApis && !date && artist && songSelected && (
					<>
						<label htmlFor="datManual" className="block text-md font-medium text-gray-700">
							Date
						</label>
						<p className="mt-2 text-sm text-gray-500" id="date-description">
				...
				{date && (
					<div className="flex justify-between text-sm">
						<p className="text-lg">{new Date(date + 'T16:00:00').toLocaleDateString()}</p>
				...
				{location && location !== '' && (
					<div className="flex justify-between text-sm">
						...
						<button type="button" onClick={showEditLocation}>
							Edit Location
						</button>
					</div>
				)}
				{((useApis && (showLocationInput || (date && !location))) || (!useApis && date)) && (
					<div>
						<label htmlFor="location" className="block text-md font-medium text-gray-700">
							Location
						</label>
						<p className="mt-2 text-sm text-gray-500" id="location-description">
				...
				{useApis && artist?.artist && (
					...
					>
						...
						<a
							href={
								artist.artist === 'Phish' || artist.artist === 'Trey Anastasio, TAB'
									? 'https://phish.net'
									: artist.artist === 'Goose'
										? 'https://elgoose.net'
						...
						{[
							...
							<p>
								Thanks{' '}
								<a className="underline" href="https://adamscheinberg.com">
									Adam Scheinberg
								</a>
								!
							</p>
				...
				{artist && songSelected && date && location && jam && jam !== 'not on jf' && (
					<SuccessAlert
				...
				{artist && songSelected && date && location && soundsSelected && soundsSelected.length > 0 && (
					<p>Sounds: {soundsSelected.join(', ')}</p>
				)}
				{/* listen link */}
				{artist &&
					songSelected &&
					...
					(!jam || (typeof jam !== 'string' && !jam?.listen_link)) && (
						<div className="mt-6">
							<label htmlFor="listen-link" className="block text-sm font-medium text-gray-700">
								Listen Link
							</label>
							<div className="mt-1">
								<input
				...
				{artist && songSelected && date && location && profile && (
					<div className="max-w-xs">
						<Listbox value={rating} onChange={handleRatingChange}>
							{({ open }) => (
								<>
									...
									<div className="relative mt-1">
										<Listbox.Button className="relative w-full cursor-default rounded-md border border-gray-300 bg-white py-2 pl-3 pr-10 text-left shadow-sm focus:border-cyan-500 focus:outline-none focus:ring-1 focus:ring-cyan-500 sm:text-sm h-10">
											<span className="block truncate">{rating || 'No rating'}</span>
											<span className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2">
												<ChevronUpDownIcon
													className="h-8 w-8 text-gray-400"
													aria-hidden="true"
										...
										>
											<Listbox.Options className="absolute z-10 mt-1 max-h-50 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm h-60">
												{[10, 9, 8, 7, 6, 5, 4, 'No rating'].map((rating, ratingIdx) => (
													<Listbox.Option
														key={ratingIdx}
														className={({ active }) =>
															classNames(
        ...
        */}
				...
				>
					...
					{!profile && jam && typeof jam !== 'string' && jam?.sounds?.length !== soundsSelected?.length && (
						<button
							type="submit"
							name="_action"
				...
				{actionData && actionData?.body === 'action complete' && (
					<div className="pb-20">
						<SuccessAlert
							title={'Success!'}
							description={'Thanks for contributing and helping other fans find this jam'}
						/>
					</div>
				)}
				{actionData && actionData?.status !== 200 && (
					<div className="pb-20">
						<InfoAlert
							title={'Error :('}
							description={'Something went wrong. Please try again and/or let me know'}
						/>
						on{' '}
