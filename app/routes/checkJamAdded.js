...
export const loader = async ({ request, params }) => {
	...
	const { data: songObj } = await supabase.from('songs').select('*').eq('song', song).single()
	...
	let shouldGetShowsByYear = queryParams?.fetchShowsByYear && queryParams?.fetchShowsByYear !== 'false'
		...
		if (artist === 'Phish' || artist === 'Trey Anastasio, TAB') {
			...
			if (setlist && setlist.data && setlist.data.length > 0) {
				...
				const titles = setlist.data
					...
					.map(({ song }) => {
						...
						const alreadyAdded = jfVersionsOnDate.find(({ song_name }) => song_name === song)
						return {
			...
			if (setlist && setlist.data && setlist.data.length > 0) {
				...
				const titles = setlist?.data
					...
					.map(({ songname }) => {
						const alreadyAdded = jfVersionsOnDate.find(({ song_name }) => song_name === songname)
			...
			if (setlist && setlist.setlist && setlist.setlist.length > 0) {
				...
				const titles = setlist.setlist[0].sets.set
					.map(({ song }) =>
						song.map(({ name }) => {
							const alreadyAdded = jfVersionsOnDate.find(({ song_name }) => song_name === name)
							if (name === '2 x 2') {
								name = '2x2'
							}
							return {
	...
	let songId
	const { data: versions, error2 } = await supabase
		.from('versions')
		.select('*')
		.eq('artist', artist)
		.eq('song_name', song)
	...
	if (artist === 'Phish' || artist === 'Trey Anastasio, TAB') {
		...
		switch (song) {
			case 'Also Sprach Zarathustra (2001)':
				...
				break
			default:
				const { data, error } = await supabase.from(tableName).select('songid').eq('song', song)
		...
		if (songId) {
			...
			shows = shows?.data
				...
				.map((show) => {
					...
					const alreadyAdded = jfVersionsOfSong.find((version) => version.date === show.showdate)
					return {
		...
		if (dbName && song) {
			const { data, error } = await supabase.from(dbName).select('id').eq('name', song)
			if (error || data.length === 0) {
					...
					.map((show) => {
						...
						return {
							...
							label: `${alreadyAdded ? '(Added) ' : ''}${show.showdate} - ${
								show.venuename
							}, ${show.city}, ${show.country === 'USA' ? show.state : show.country}`,
							existingJam: alreadyAdded ?? null,
						}
					})
				shows = shows.reverse()
			...
			if (mbid && year) {
				...
				shows = data.map((show) => {
					const location = `${show?.venue?.name ? show.venue.name + ', ' : ''}${show.venue.city.name}, ${
						show.venue.city.country.code === 'US' ? show.venue.city.state : show.venue.city.country.name
					}`
