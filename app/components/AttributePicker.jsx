export default function AttributePicker({
	...
	function isDefaultChecked(attribute) {
		if (attributeFilters) {
			return attributeFilters.includes(JSON.stringify(attribute))
		}
	}
	return (
				...
				<div className="mt-1 flex rounded-md shadow-sm">
					<fieldset>
						...
						<div className="divide-y divide-gray-200 border-t border-b border-gray-200 max-h-60 overflow-y-scroll sm:col-span-6">
							{attributes &&
								attributes?.map((sound) => (
									<div key={sound.id} className="relative flex items-start py-4">
										...
										<div className="ml-3 flex h-5 items-center">
											<input
												...
												defaultChecked={isDefaultChecked(sound)}
											/>
										</div>
									</div>
								))}
